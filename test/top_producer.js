const Producer = artifacts.require("./TopProducer.sol");
const PaymentReceiver = artifacts.require("./TopPaymentReceiver.sol");
...
contract("Producer", async () => {
	...
	let producer = await Producer.new(owner, deployer, taxman, store);
	...
	let ev = await producer.PaymentReceiverDeployed('latest', async (error, result) => {
			if (!error) {
				...
				let tx = await receiver.withdrawTokens({from:taxman});
